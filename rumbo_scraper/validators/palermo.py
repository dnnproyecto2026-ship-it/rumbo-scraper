"""Validation rules for the Universidad de Palermo scrape."""

from rumbo_scraper.contracts import SECTION_FIELDS
from rumbo_scraper.parsers.palermo import UNIVERSITY
...
def validate_dataset(dataset: dict[str, object]) -> None:
    ...
    if not isinstance(sections, dict) or set(sections) != set(SECTION_FIELDS):
        raise ValueError("El dataset de Palermo no respeta el contrato de secciones.")
    ...
    for label, values in (("carreras", names), ("posgrados", programmes)):
        if len(values) != len(set(values)):
            ...
            raise ValueError(f"Hay {label} duplicados en Palermo: {repeated[:3]}")
