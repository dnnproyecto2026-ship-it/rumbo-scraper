...
from rumbo_scraper.parsers.umsa import UNIVERSITY
...
def validate_dataset(dataset: dict[str, object]) -> None:
    ...
    if not isinstance(sections, dict) or set(sections) != set(SECTION_FIELDS):
        raise ValueError("El dataset de UMSA no respeta el contrato de secciones.")
    ...
    if len(names) != len(set(names)):
        raise ValueError("Hay carreras duplicadas en UMSA.")
    if len(programmes) != len(set(programmes)):
        raise ValueError("Hay posgrados duplicados en UMSA.")
