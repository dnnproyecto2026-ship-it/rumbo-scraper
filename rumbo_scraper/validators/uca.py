...
from rumbo_scraper.parsers.uca import UNIVERSITY
...
def validate_dataset(dataset: dict[str, object]) -> None:
    ...
    if not isinstance(sections, dict) or set(sections) != set(SECTION_FIELDS):
        raise ValueError("El dataset de UCA no respeta el contrato de secciones.")
    ...
    if len(names) != len(set(names)):
        raise ValueError("Hay carreras duplicadas en UCA.")
    if len(programmes) != len(set(programmes)):
        raise ValueError("Hay posgrados duplicados en UCA.")
