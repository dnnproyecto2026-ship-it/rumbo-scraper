"""Validate and load the ITBA dataset into Supabase."""
...
DEFAULT_INPUT = Path("data/itba_completo.json")
CORE_SECTIONS = (
    "localidades", "universidades", "sedes", "carreras", "ofertas",
    "materias", "posgrados", "autoridades",
)
...
def apply_dataset(dataset: dict[str, Any], client: Any | None = None) -> dict[str, int]:
    ...
    for row in data["carreras"]:
        saved = _upsert_one(client, "carreras", {
            "universidad_id": university_id, "facultad_id": None,
    ...
    # ofertas_academicas.sede_id is NOT NULL, and every ITBA degree publishes
    # "Multisede": taught across campuses without stating which. Choosing one
    # would assert something the university does not, so those offers stay out
    # of the database and are counted here.
    ...
    # autoridades.facultad_id is NOT NULL and ITBA publishes its authorities at
    # the level of the university, not of a faculty, so they have no row there.
    counts["autoridades_sin_facultad"] = len(data["autoridades"])
    return counts


def main() -> None:
    parser = argparse.ArgumentParser(description="Cargar el dataset de ITBA")
