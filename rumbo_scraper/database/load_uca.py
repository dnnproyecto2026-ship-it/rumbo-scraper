"""Validate and load the UCA dataset into Supabase."""
...
DEFAULT_INPUT = Path("data/uca_completo.json")
CORE_SECTIONS = (
...
def apply_dataset(dataset: dict[str, Any], client: Any | None = None) -> dict[str, int]:
    ...
    # UCA does not state which campus teaches each programme on its pages, and
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cargar el dataset de UCA")
